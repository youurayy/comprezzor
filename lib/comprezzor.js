...
module.exports = function(compress, opts) {
    ...
    if(zo.pool) {
        ...
        zo.pool.create = _x(null, false, function(cb) { // cb(err, res)
            var z = module.exports(compress, zo);
            cb(null, z);
        });

        var pool = genpool.Pool(zo.pool);
        delete zo.pool;

        return {

            make: _x(null, false, function(str, cb) {
                pool.acquire(_x(cb, true, function(err, z) {
    ...
    this.make = function(string, __cb) { // __cb(err, res)
        if(_cb)
            _e('currently in use');
        _cb = __cb;
        this.init();
